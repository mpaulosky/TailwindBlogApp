...
public class CategoryService : ICategoryService
{
	...
	/// <returns>A <see cref="Result{T}"/> containing a <see cref="CategoryDto"/> if successful, or a failure result with an error message.</returns>
	...
	public async Task<Result<List<CategoryDto>>> GetAllAsync()
	{
		...
		if (output is not null) return Result<List<CategoryDto>>.Ok(output);

		var results = await _repository.GetAllAsync();

		// If the repository call fails, return the failure result
		if (results.Failure) return Result<List<CategoryDto>>.Fail("Failed to retrieve categories.");
